...
# --- SELECTORES DE MERCADO Y PERIODO ---
st.sidebar.subheader("2. Estrategia de Caza")

# Tipo de apuesta (MERCADOS AÑADIDOS AQUÍ)
...
# 1. Prefijo (MERCADOS MAPEADOS AQUÍ)
api_market = "h2h"
...
if st.button("🚀 BUSCAR CUOTAS", type="primary"):
    ...
    with st.spinner(f"Rastreando {bet_type} para {period_type} en todo el mundo..."):
        ...
        if error:
            st.error(f"Error API o Mercado no disponible en tu plan: {error}")
        elif not data:
            st.warning(f"No hay cuotas activas para '{period_type}' o este mercado no está soportado. Intenta con Partido Completo o revisa tu plan.")
        else:
            ...
            for game in data:
                ...
                for book in game['bookmakers']:
                    ...
                    for market in book['markets']:
                        # Comparamos con la key exacta construida
